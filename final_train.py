...
def train_nerf(
    ...
    for it in tqdm(range(1, n_iters + 1)):
        ...
        if it in [200, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000, 6500, 7000, 7500, 8000, 8500, 9000, 9500, 10000]:
            with torch.no_grad():
    ...
    shrink = 0.8                           # shrink radius for cleaner GIF
    ...
    print("🎥 Orbit A: rotating around WORLD Y-axis ...")

    scene_center = np.array([0.04124536, -0.12361822, 0.29642041], dtype=np.float32)
    ...
    base_eye = c2ws_val[0][:3, 3]
    radius_vec = base_eye - scene_center

    orbit_frames_y = []
    for i in tqdm(range(20), desc="Orbit-Y"):
        theta = 2 * np.pi * i / 20
        v_rot = rot_y(theta) @ radius_vec
        new_eye = scene_center + v_rot

        c2w = look_at(new_eye, scene_center)
        img = render_full_image(model, K, c2w, H, W, n_samples, near, far, device)
        orbit_frames_y.append((img * 255).astype(np.uint8))

    iio.imwrite(f"{results_dir}/orbit_y.gif", orbit_frames_y, format="GIF", loop=0, fps=8)
    print("✔ Saved orbit_y.gif")
